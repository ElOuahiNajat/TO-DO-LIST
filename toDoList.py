...
class TodoListApp(tk.Tk):
    def __init__(self):
        ...
        # Bind double-click event to task list
    ...
    def edit_task(self, event):
        ...
        task_index = self.task_list.nearest(event.y)
        # Get text of selected task
        task_text = self.task_list.get(task_index)
        # Open a dialog box to edit the task
        edited_text = messagebox.askstring("Edit Task", "Edit Task:", initialvalue=task_text)
        if edited_text is not None:
            self.task_list.delete(task_index)
            self.task_list.insert(task_index, edited_text)
            self.save_tasks()
